                ...
                for image in blog_images:
                    ...
                    if image_path.exists():
                        ...
                        destination_path = (
                            ...
                            / f"authors/images/{image_filename}"
                        )
...
def update_index_file(sphinx_app: Sphinx, rocm_blogs: ROCmBlogs = None) -> None:
    ...
    global _CRITICAL_ERROR_OCCURRED, traceback
    phase_start_time = time.time()
    ...
    try:
        ...
        external_sidebar_css = import_file(
            "rocm_blogs.static.css", "external_sidebar.css"
        )
        ...
        index_template = INDEX_TEMPLATE.format(
            CSS=css_content,
            BANNER_CSS=banner_css_content,
            EXTERNAL_SIDEBAR_CSS=external_sidebar_css,
            HTML=template_html,
        )
        ...
        try:
            ...
            log_message(
                "info",
                "External content system initialized successfully",
                "external_content",
                "__init__",
            )
            if log_file_handle:
            ...
            log_message(
                "warning",
                f"Failed to initialize external content: {e}",
                "external_content",
                "__init__",
            )
            if log_file_handle:
                safe_log_write(
                    log_file_handle,
                    f"WARNING: External content initialization failed: {e}\n",
                )
        ...
        try:
            ...
            if external_items:
                ...
                log_message(
                    "info",
                    f"Generated {len(cards)} external content cards",
                    "external_content",
                    "__init__",
                )
                if log_file_handle:
            ...
            log_message(
                "warning",
                f"Failed to generate external content: {e}\nTraceback: {error_details}",
                "external_content",
                "__init__",
            )
            if log_file_handle:
                safe_log_write(
                    log_file_handle,
                    f"WARNING: Failed to generate external content: {e}\n",
                )
