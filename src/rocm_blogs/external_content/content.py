...
class ExternalContentLoader:
    ...
    def load_from_csv(self, csv_filename: str = "external-content.csv") -> List[ExternalContent]:
        ...
        csv_paths = [
            self.blogs_directory.parent / csv_filename,  # In parent directory (project root) - try this first
        ...
        if csv_path and csv_path.exists():
            log_message("info", f"Found external content CSV at: {csv_path}", 
                       "external_content", "csv_loading")
        else:
            log_message("info", f"Searching for external content CSV, tried: {[str(p) for p in csv_paths]}", 
                       "external_content", "csv_loading")
