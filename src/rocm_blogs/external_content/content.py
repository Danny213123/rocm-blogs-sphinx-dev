...
class ExternalContent:
    ...
    def __init__(self, data: Dict[str, str], row_number: Optional[int] = None):
        ...
        log_message(
            "debug",
            f"Initializing ExternalContent from row {row_number or 'unknown'}",
            "external_content",
            "content_init",
        )
        ...
        log_message(
            "debug",
            f"Parsed content fields - Title: '{self.title}', Type: '{self.content_type}', Category: '{self.category}'",
            "external_content",
            "content_init",
        )
        # Parse date
        ...
        try:
            ...
            log_message(
                "debug",
                f"Successfully parsed date: {date_str} -> {self.date.strftime('%Y-%m-%d')}",
                "external_content",
                "date_parsing",
            )
            ...
            log_message(
                "warning",
                f"Invalid date format for external content row {row_number}: '{date_str}' - {e}",
                "external_content",
                "date_parsing",
            )
            self.date = None

        # Extract domain from URL if source_domain not provided
        if not self.source_domain and self.url:
            try:
                ...
                log_message(
                    "debug",
                    f"Extracted source domain from URL: {self.source_domain}",
                    "external_content",
                    "domain_extraction",
                )
                ...
                log_message(
                    "warning",
                    f"Failed to extract domain from URL '{self.url}': {e}",
                    "external_content",
                    "domain_extraction",
                )
    ...
    def is_valid(self) -> bool:
        ...
        if missing_fields:
            log_message(
                "debug",
                f"Missing required fields for external content: {missing_fields}",
                "external_content",
                "validation",
            )
    ...
    def get_content_type_icon(self) -> str:
        ...
        log_message(
            "debug",
            f"Content type icon for '{self.content_type}': {icon}",
            "external_content",
            "icon_mapping",
        )
        return icon
...
class ExternalContentLoader:
    ...
    def __init__(self, blogs_directory: str):
        ...
        log_message(
            "info",
            f"Initializing ExternalContentLoader with directory: {self.blogs_directory}",
            "external_content",
            "loader_init",
        )

    def load_from_csv(
        self, csv_filename: str = "external-content.csv"
    ) -> List[ExternalContent]:
        """Load external content from CSV file."""
        # Try multiple locations for the CSV file
        csv_paths = [
            self.blogs_directory.parent
            / csv_filename,  # In parent directory (project root) - try this first
            self.blogs_directory / csv_filename,  # In blogs directory
            Path(csv_filename),  # Current working directory
            Path.cwd() / csv_filename,  # Explicit current working directory
        ]

        # If blogs_directory has 'blogs' in the path, also try going up two levels
        if "blogs" in str(self.blogs_directory):
            csv_paths.insert(0, self.blogs_directory.parent.parent / csv_filename)
        ...
        if csv_path is None:
            csv_path = csv_paths[1]  # Default to blogs directory for error message

        # Create dedicated log file for external content loading
        log_file_path, log_file_handle = create_step_log_file(
            "external_content_loading"
        )

        if csv_path and csv_path.exists():
            log_message(
                "info",
                f"Found external content CSV at: {csv_path}",
                "external_content",
                "csv_loading",
            )
        else:
            log_message(
                "info",
                f"Searching for external content CSV, tried: {[str(p) for p in csv_paths]}",
                "external_content",
                "csv_loading",
            )
        ...
        if log_file_handle:
            safe_log_write(
                log_file_handle, f"=== External Content Loading Session ===\n"
            )
        ...
        try:
            with open(csv_path, "r", newline="", encoding="utf-8") as file:
                ...
                headers_msg = (
                    f"CSV headers: {list(fieldnames) if fieldnames else 'None'}"
                )
                ...
                for row_num, row in enumerate(reader, start=1):
                    ...
                    log_message(
                        "debug",
                        f"Processing CSV row {row_num}: {dict(row)}",
                        "external_content",
                        "csv_parsing",
                    )
                    if log_file_handle:
                    ...
                    if content.is_valid():
                        ...
                        log_message(
                            "debug", success_msg, "external_content", "content_loading"
                        )
                        if log_file_handle:
                            safe_log_write(
                                log_file_handle, f"  ✓ LOADED: {success_msg}\n"
                            )
                    else:
                        skipped_count += 1
                        ...
                        log_message(
                            "warning", error_msg, "external_content", "content_loading"
                        )
                        if log_file_handle:
                            safe_log_write(
                                log_file_handle, f"  ✗ SKIPPED: {error_msg}\n"
                            )
            ...
            if log_file_handle:
                safe_log_write(log_file_handle, f"FATAL ERROR: {error_msg}\n")
                safe_log_close(log_file_handle)
            # Always ensure we have some content to display even if CSV loading fails
            # This prevents the sidebar from being completely empty
            if not external_content:
                log_message(
                    "warning",
                    "No external content loaded, using fallback empty list",
                    "external_content",
                    "csv_loading",
                )
        ...
        if external_content:
            # Log category and content type breakdown
            categories = {}
            content_types = {}
            domains = {}
            ...
            for content in external_content:
                ...
                content_types[content.content_type] = (
                    content_types.get(content.content_type, 0) + 1
                )
        ...
        if log_file_path:
            log_message(
                "info",
                f"External content detailed log written to: {log_file_path}",
                "external_content",
                "csv_loading",
            )
    ...
    def get_by_category(self, category: str) -> List[ExternalContent]:
        ...
        log_message(
            "debug",
            f"Filtering external content by category: '{category}'",
            "external_content",
            "filtering",
        )
        ...
        log_message(
            "info",
            f"Found {len(filtered_content)} items for category '{category}'",
            "external_content",
            "filtering",
        )
    ...
    def get_recent(self, limit: int = 5) -> List[ExternalContent]:
        ...
        log_message(
            "debug",
            f"Getting {limit} most recent external content items",
            "external_content",
            "filtering",
        )
        ...
        log_message(
            "debug",
            f"Content with dates: {len(dated_content)}, without dates: {len(undated_content)}",
            "external_content",
            "filtering",
        )

        sorted_content = sorted(dated_content, key=lambda x: x.date, reverse=True)
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        log_message(
            "debug",
            "Generating external content statistics",
            "external_content",
            "statistics",
        )
        ...
        for content in self.external_content:
            ...
            # Content types
            stats["content_types"][content.content_type] = (
                stats["content_types"].get(content.content_type, 0) + 1
            )
        ...
        log_message(
            "info",
            f"Generated statistics for {stats['total_items']} external content items",
            "external_content",
            "statistics",
        )
