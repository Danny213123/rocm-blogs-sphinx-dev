...
def generate_external_item_html(content: ExternalContent) -> str:
    ...
    log_message("debug", f"Generating HTML for external item: '{content.title}'", 
               "external_content", "html_generation")
    ...
    if content.date:
        try:
            ...
            log_message("debug", f"Formatted date for '{content.title}': {date_str}", 
                       "external_content", "date_formatting")
        except Exception as e:
            log_message("warning", f"Error formatting date for '{content.title}': {e}", 
                       "external_content", "date_formatting")
            date_str = ""
    ...
    if content.author:
        ...
        log_message("debug", f"Added author for '{content.title}': {content.author}", 
                   "external_content", "html_generation")
    ...
    log_message("debug", f"Built meta HTML for '{content.title}': {len(meta_parts)} parts", 
               "external_content", "html_generation")
    ...
    if content.source_domain:
        ...
        log_message("debug", f"Added source domain for '{content.title}': {content.source_domain}", 
                   "external_content", "html_generation")
    
    if not content.url:
        log_message("warning", f"External content '{content.title}' has no URL", 
                   "external_content", "html_generation")
        return ""
    
    if not content.url.startswith(('http://', 'https://')):
        log_message("warning", f"External content '{content.title}' has invalid URL: {content.url}", 
                   "external_content", "html_generation")
    
    description_html = ""
    if content.description:
        description_html = f'<p class="external-article-description">{content.description}</p>'
        log_message("debug", f"Added description for '{content.title}': {len(content.description)} chars", 
                   "external_content", "html_generation")
    ...
    log_message("debug", f"Generated HTML for '{content.title}': {len(item_html)} characters", 
               "external_content", "html_generation")
...
def generate_external_sidebar_content(
    ...
    log_message("info", f"Generating external sidebar content with {len(external_items)} items (max: {max_items}, descriptions: {show_descriptions})", 
               "external_content", "sidebar_generation")
    
    if log_file_handle:
        safe_log_write(log_file_handle, f"=== External Content Sidebar Generation ===\n")
    ...
    log_message("debug", f"Sorting external items - Dated: {len(dated_items)}, Undated: {len(undated_items)}", 
               "external_content", "sidebar_generation")
    ...
    try:
        ...
        log_message("debug", f"Successfully sorted {len(sorted_items)} dated items", 
                   "external_content", "sidebar_generation")
    except Exception as e:
        log_message("error", f"Error sorting external content by date: {e}", 
                   "external_content", "sidebar_generation")
    ...
    log_message("info", f"Selected {len(display_items)} items for display (from {len(sorted_items)} total)", 
               "external_content", "sidebar_generation")
    ...
    for i, item in enumerate(display_items):
        try:
            log_message("debug", f"Processing item {i+1}/{len(display_items)}: '{item.title}'", 
                       "external_content", "item_processing")
            
            item_html = generate_external_item_html(item)
            
            if not item_html:
                log_message("warning", f"Generated empty HTML for external item: '{item.title}'", 
                           "external_content", "item_processing")
                failed_items += 1
                continue
            
            if not show_descriptions and '<p class="external-article-description">' in item_html:
                ...
                log_message("debug", f"Removed description from '{item.title}': {original_length} -> {len(item_html)} chars", 
                           "external_content", "item_processing")
            ...
            failed_items += 1
            log_message("error", f"Failed to generate external item HTML for '{item.title}': {e}", 
                       "external_content", "item_processing")
    
    if failed_items > 0:
        log_message("warning", f"Failed to generate HTML for {failed_items} external content items", 
                   "external_content", "sidebar_generation")
    
    final_html = '\n'.join(items_html)
    
    summary_msg = f"Generated sidebar HTML: {len(items_html)} items, {len(final_html)} characters"
    ...
    if log_file_handle:
        ...
        safe_log_write(log_file_handle, f"Final HTML length: {len(final_html)} characters\n")
        safe_log_write(log_file_handle, f"Session complete: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    if log_file_path:
        log_message("info", f"Sidebar generation detailed log written to: {log_file_path}", 
                   "external_content", "sidebar_generation")
...
def generate_external_content(
    ...
    log_message("info", f"Generating homepage with external content - Main title: '{main_title}', Max external: {max_external}", 
               "external_content", "homepage_generation")
    
    log_message("debug", f"Main grid items length: {len(main_grid_items)} chars", 
               "external_content", "homepage_generation")
    
    external_items_html = generate_external_sidebar_content(
        external_content, 
        max_items=max_external,
        show_descriptions=True
    )
    
    if not external_items_html:
        log_message("warning", "No external items HTML generated for homepage", 
                   "external_content", "homepage_generation")
...
<div class="homepage-container">
    ...
    log_message("info", f"Generated complete homepage HTML: {len(final_homepage)} characters", 
               "external_content", "homepage_generation")
...
def validate_external_content_html(html: str, content_title: str = "unknown") -> bool:
    ...
    log_message("debug", f"Validating HTML for external content: '{content_title}'", 
               "external_content", "html_validation")
    ...
    if open_tags != close_tags:
        issues.append(f"Potential tag mismatch: {open_tags} opening tags, {close_tags} closing tags")
    
    if issues:
        log_message("warning", f"HTML validation issues for '{content_title}': {', '.join(issues)}", 
                   "external_content", "html_validation")
        return False
    else:
        log_message("debug", f"HTML validation passed for '{content_title}'", 
                   "external_content", "html_validation")
        return True