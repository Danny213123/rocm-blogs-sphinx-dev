...
class BlogHolder:
    ...
    def initialize_external_content(self, blogs_directory: str) -> None:
        ...
        log_message("info", f"Initializing external content loader with directory: {blogs_directory}", "external_content", "holder")
    ...
    def get_recent_external_content(self, limit: int = 5):
        ...
        if not self.external_content_loader:
            log_message("warning", "External content loader not initialized", "external_content", "holder")
            return []
        
        recent_content = self.external_content_loader.get_recent(limit)
        log_message("info", f"Retrieved {len(recent_content)} recent external content items", "external_content", "holder")
